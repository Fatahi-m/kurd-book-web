  ...
  return (
    <Link href={`/arts/${product.id}`} className="group block">
      <div className="relative aspect-[4/5] overflow-hidden rounded-lg bg-gray-100 mb-4">
        ...
        {product.isHandmade && (
          <div className="absolute top-3 right-3 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full text-xs font-medium uppercase tracking-wider text-gray-900 shadow-sm">
            Handmade
          </div>
        )}
      </div>
      ...
      <div className="space-y-1">
        <h3 className="text-lg font-serif text-gray-900 group-hover:text-[#e11d48] transition-colors">
          {title}
        </h3>
        {artisan && (
          <p className="text-sm text-gray-500 font-light">
            {t('book.by')} <span className="font-medium text-gray-700">{artisanName}</span>
          </p>
        )}
